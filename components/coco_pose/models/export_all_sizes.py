...
def export_and_rename(size, target_name):
    print(f"导出{size}尺寸的模型...")
    ...
    model.export(format="onnx", simplify=True, opset=13, dynamic=False, imgsz=size)
    
    # 重命名文件
    if os.path.exists("yolo11n-pose.onnx"):
        os.rename("yolo11n-pose.onnx", target_name)
        print(f"模型已保存为: {target_name}")


# 导出256尺寸的模型 (240会自动调整为256)
export_and_rename(240, "yolo11n-pose-256.onnx")

# 导出224尺寸的模型 (200会自动调整为224)  
export_and_rename(200, "yolo11n-pose-224.onnx")
