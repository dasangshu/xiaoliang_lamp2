...
static bool hw_jpeg_memory_available(void) {
...
static bool hw_jpeg_ensure_inited(void) {
    ...
    if (!hw_jpeg_memory_available()) {
        ESP_LOGW(TAG, "insufficient internal DMA memory for HW JPEG, using SW encoder");
    ...
    if (er != ESP_OK) {
        ESP_LOGW(TAG, "HW JPEG init failed (%s), using SW encoder", esp_err_to_name(er));
...
void image_to_jpeg_hw_warmup(void) {
    if (hw_jpeg_ensure_inited()) {
        ESP_LOGI(TAG, "HW JPEG encoder ready");
    }
}
